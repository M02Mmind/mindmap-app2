...
// ─── Block-Editor: Datenmodell & Helfer ───────────────────────────────────────
// Jeder Knoten besitzt jetzt optional ein `blocks`-Array (freier Block-Editor im Notion-Stil).
// Blocktypen: 'text', 'heading', 'todo', 'quote', 'divider', 'image', 'video', 'file'.
let _blockUid = 900;
const buid = () => 'blk' + (_blockUid++);

function makeBlock(type, extra = {}) {
  return { id: buid(), type, content: '', checked: false, ...extra };
}

// Migriert die alten getrennten Felder (definition, note, images, videos, files) einmalig
// in ein Blocks-Array, damit bereits bestehende Knoten nichts verlieren, wenn sie zum
// ersten Mal im neuen Block-Editor geöffnet werden.
function ensureBlocks(node) {
  if (node.blocks) return node.blocks;
  const blocks = [];
  if (node.definition && node.definition.trim()) {
    blocks.push(makeBlock('heading', { content: 'Definition' }));
    blocks.push(makeBlock('text', { content: node.definition }));
  }
  if (node.note && node.note.trim()) {
    blocks.push(makeBlock('heading', { content: 'Notizen' }));
    blocks.push(makeBlock('quote', { content: node.note }));
  }
  (node.images || []).forEach(img => blocks.push(makeBlock('image', { src: img.src, caption: img.caption })));
  (node.videos || []).forEach(vid => blocks.push(makeBlock('video', { src: vid.src, caption: vid.caption })));
  (node.files || []).forEach(f => blocks.push(makeBlock('file', { src: f.src, caption: f.caption, size: f.size, mime: f.mime })));
  if (blocks.length === 0) blocks.push(makeBlock('text'));
  node.blocks = blocks;
  return blocks;
}

// Definition der Slash-Command-Optionen: Label, Beschreibung, Icon, und wie der Block erzeugt wird
const SLASH_COMMANDS = [
  { key:'text',    label:'Text',          desc:'Einfacher Textabsatz',        icon:'📝', make:()=>makeBlock('text') },
  { key:'heading', label:'Überschrift',   desc:'Abschnittstitel',             icon:'🔠', make:()=>makeBlock('heading') },
  { key:'todo',    label:'Checkliste',    desc:'Aufgabe zum Abhaken',         icon:'☑️', make:()=>makeBlock('todo') },
  { key:'quote',   label:'Zitat',         desc:'Hervorgehobener Textblock',   icon:'❝',  make:()=>makeBlock('quote') },
  { key:'divider', label:'Trennlinie',    desc:'Visuelle Trennung',           icon:'―',  make:()=>makeBlock('divider') },
  { key:'bullet',   label:'Stichpunkte',   desc:'Aufzählung mit Punkten',      icon:'•',  make:()=>makeBlock('bullet') },
  { key:'numbered', label:'Nummerierung',  desc:'1., 2., 3., ...',             icon:'1.', make:()=>makeBlock('numbered') },
  { key:'image',   label:'Bild',          desc:'Bild hochladen',              icon:'🖼️', make:()=>makeBlock('image', { pendingUpload:'image' }) },
  { key:'video',   label:'Video',         desc:'Video hochladen',             icon:'🎬', make:()=>makeBlock('video', { pendingUpload:'video' }) },
  { key:'file',    label:'Datei',         desc:'PDF, Word, ZIP, ...',         icon:'📎', make:()=>makeBlock('file', { pendingUpload:'file' }) },
];
...
// ─── Einzelner Block: rendert je nach Typ ein anderes Editier-Element ────────
function BlockRow({ block, onChange, onRemove, onEnter, onBackspaceEmpty, onSlash, autoFocus, theme, accentColor, listNumber }) {
  ...
  const commonTextProps = {
    ...
    onChange: e => {
      ...
      if (val.endsWith('/') && val.length === 1) onSlash(true);
      else if (onSlash) onSlash(false);
    },
    onKeyDown: e => {
      if (e.key === 'Enter' && (block.type === 'heading')) { e.preventDefault(); onEnter(); return; }
      if (e.key === 'Enter' && !e.shiftKey && (block.type === 'text' || block.type === 'quote' || block.type === 'todo' || block.type === 'bullet' || block.type === 'numbered')) {
        e.preventDefault(); onEnter(); return;
      }
      if (e.key === 'Backspace' && block.content === '') { e.preventDefault(); onBackspaceEmpty(); }
    },
    placeholder: block.type === 'heading' ? 'Überschrift...' : block.type === 'quote' ? 'Zitat oder Notiz...' : (block.type === 'bullet' || block.type === 'numbered') ? 'Punkt eingeben...' : 'Schreib etwas, oder "/" für Befehle...',
    style: {
      width:'100%', border:'none', outline:'none', resize:'none', background:'transparent',
      fontFamily:'inherit', color: theme?.text || '#1F1E1C',
      fontSize: block.type === 'heading' ? 18 : 14,
      fontWeight: block.type === 'heading' ? 700 : 400,
      lineHeight: 1.6, padding: 0, overflow:'hidden',
    },
  };
  ...
  const handleUpload = async (files, kind) => {
    ...
    if (kind === 'file') {
      onChange({ ...block, src, caption:file.name, size:file.size, mime:file.type, pendingUpload:undefined });
    } else {
      onChange({ ...block, src, caption:file.name, pendingUpload:undefined });
    }
  };

  if (block.type === 'divider') {
    return (
      <div style={{position:'relative', padding:'10px 0', display:'flex', alignItems:'center', gap:8, group:true}}>
        <div style={{flex:1, height:1, background: theme?.border || '#E6E4DF'}}/>
        <button onClick={onRemove} title="Entfernen" style={{background:'transparent', border:'none', color:'#B3B0A9', cursor:'pointer', fontSize:12}}>✕</button>
      </div>
    );
  }
    ...
    return (
      <div style={{display:'flex', alignItems:'flex-start', gap:8, padding:'3px 0'}}>
        <input type="checkbox" checked={!!block.checked} onChange={e=>onChange({...block, checked:e.target.checked})}
          style={{marginTop:5, width:15, height:15, accentColor: accentColor, cursor:'pointer', flexShrink:0}}/>
        <textarea {...commonTextProps} rows={1}
          ref={el => { ref.current = el; autoResize(el); }}
          onInput={e=>autoResize(e.target)}
          style={{...commonTextProps.style, textDecoration: block.checked ? 'line-through' : 'none', opacity: block.checked ? 0.55 : 1}}/>
        <button onClick={onRemove} title="Entfernen" style={{background:'transparent', border:'none', color:'#B3B0A9', cursor:'pointer', fontSize:12, flexShrink:0}}>✕</button>
      </div>
    );
  }
    ...
    return (
      <div style={{display:'flex', gap:10, padding:'4px 0'}}>
        ...
        <textarea {...commonTextProps} rows={1}
          ref={el => { ref.current = el; autoResize(el); }}
          onInput={e=>autoResize(e.target)}
          style={{...commonTextProps.style, fontStyle:'italic', color: theme?.textSoft || '#6B6963'}}/>
        <button onClick={onRemove} title="Entfernen" style={{background:'transparent', border:'none', color:'#B3B0A9', cursor:'pointer', fontSize:12, flexShrink:0}}>✕</button>
      </div>
    );
  }
  ...
  if (block.type === 'heading') {
    return (
      <div style={{display:'flex', alignItems:'flex-start', gap:8, padding:'8px 0 2px'}}>
        <textarea {...commonTextProps} rows={1}
          ref={el => { ref.current = el; autoResize(el); }}
          onInput={e=>autoResize(e.target)}/>
        <button onClick={onRemove} title="Entfernen" style={{background:'transparent', border:'none', color:'#B3B0A9', cursor:'pointer', fontSize:12, flexShrink:0, marginTop:4}}>✕</button>
      </div>
    );
  }
  ...
  if (block.type === 'image') {
    if (!block.src) {
      return (
        <div className="drop-zone" style={{marginBottom:2}}
          onClick={()=>fileRef.current?.click()}
          onDragOver={e=>{e.preventDefault();e.currentTarget.classList.add('drag-over');}}
          onDragLeave={e=>e.currentTarget.classList.remove('drag-over')}
          onDrop={e=>{e.preventDefault();e.currentTarget.classList.remove('drag-over');handleUpload(Array.from(e.dataTransfer.files),'image');}}>
          🖼️ Bild hinzufügen
          <input ref={fileRef} type="file" accept="image/*" style={{display:'none'}}
            onChange={e=>{handleUpload(Array.from(e.target.files),'image'); e.target.value='';}}/>
        </div>
      );
    }
    return (
      <div style={{position:'relative', display:'inline-block', marginBottom:2}} className="img-thumb">
        <img src={block.src} alt={block.caption} style={{maxWidth:'100%', maxHeight:320, borderRadius:8, border:'0.5px solid '+(theme?.border||'#D7D4CC'), display:'block'}}/>
        <button className="img-del" onClick={onRemove} title="Entfernen">✕</button>
      </div>
    );
  }
    ...
    if (!block.src) {
      return (
        <div className="drop-zone" style={{marginBottom:2}}
          ...
          onDragOver={e=>{e.preventDefault();e.currentTarget.classList.add('drag-over');}}
          onDragLeave={e=>e.currentTarget.classList.remove('drag-over')}
          onDrop={e=>{e.preventDefault();e.currentTarget.classList.remove('drag-over');handleUpload(Array.from(e.dataTransfer.files),'video');}}>
          🎬 Video hinzufügen
          <input ref={fileRef} type="file" accept="video/*" style={{display:'none'}}
            onChange={e=>{handleUpload(Array.from(e.target.files),'video'); e.target.value='';}}/>
        </div>
      );
    }
    return (
      <div style={{position:'relative', display:'inline-block', marginBottom:2}} className="img-thumb">
        <video src={block.src} controls style={{maxWidth:'100%', maxHeight:320, borderRadius:8, border:'0.5px solid '+(theme?.border||'#D7D4CC'), display:'block'}}/>
        <button className="img-del" onClick={onRemove} title="Entfernen">✕</button>
      </div>
    );
  }
    ...
    if (!block.src) {
      return (
        <div className="drop-zone" style={{marginBottom:2}}
          ...
          onDragOver={e=>{e.preventDefault();e.currentTarget.classList.add('drag-over');}}
          onDragLeave={e=>e.currentTarget.classList.remove('drag-over')}
          onDrop={e=>{e.preventDefault();e.currentTarget.classList.remove('drag-over');handleUpload(Array.from(e.dataTransfer.files),'file');}}>
          📎 Datei hinzufügen (PDF, Word, ZIP, ...)
          <input ref={fileRef} type="file" style={{display:'none'}}
            onChange={e=>{handleUpload(Array.from(e.target.files),'file'); e.target.value='';}}/>
        </div>
      );
    }
    return (
      <div style={{display:'flex', alignItems:'center', gap:8, padding:'7px 8px', borderRadius:8, border:'0.5px solid '+(theme?.borderStrong||'#D7D4CC'), background:theme?.surface||'#fff', marginBottom:2}}>
        <span style={{fontSize:18, flexShrink:0}}>{fileIcon(block.caption||'', block.mime||'')}</span>
        <div style={{flex:1, minWidth:0}}>
          <div style={{fontSize:12.5, fontWeight:500, color:theme?.text||'#1F1E1C', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{block.caption}</div>
          <div style={{fontSize:10.5, color:'#9B9994'}}>{formatBytes(block.size)}</div>
        </div>
        <a href={block.src} download={block.caption} title="Herunterladen" style={{fontSize:13, color: accentColor, textDecoration:'none', padding:'4px 6px', flexShrink:0}}>⬇</a>
        <button onClick={onRemove} title="Entfernen" style={{background:'transparent', border:'none', color:'#9B9994', cursor:'pointer', fontSize:13, padding:'4px 4px', flexShrink:0}}>✕</button>
      </div>
    );
  }
  ...
  if (block.type === 'bullet') {
    return (
      <div style={{display:'flex', alignItems:'flex-start', gap:8, padding:'2px 0'}}>
        <span style={{fontSize:15, lineHeight:1.6, color: accentColor, fontWeight:700, flexShrink:0, marginTop:1, userSelect:'none'}}>•</span>
        <textarea {...commonTextProps} rows={1}
          ref={el => { ref.current = el; autoResize(el); }}
          onInput={e=>autoResize(e.target)}/>
        <button onClick={onRemove} title="Entfernen" style={{background:'transparent', border:'none', color:'#B3B0A9', cursor:'pointer', fontSize:12, flexShrink:0, marginTop:2}}>✕</button>
      </div>
    );
  }
  ...
  if (block.type === 'numbered') {
    return (
      <div style={{display:'flex', alignItems:'flex-start', gap:8, padding:'2px 0'}}>
        <span style={{fontSize:13, lineHeight:1.6, color: accentColor, fontWeight:600, flexShrink:0, marginTop:1, minWidth:20, textAlign:'right', userSelect:'none'}}>{listNumber}.</span>
        <textarea {...commonTextProps} rows={1}
          ref={el => { ref.current = el; autoResize(el); }}
          onInput={e=>autoResize(e.target)}/>
        <button onClick={onRemove} title="Entfernen" style={{background:'transparent', border:'none', color:'#B3B0A9', cursor:'pointer', fontSize:12, flexShrink:0, marginTop:2}}>✕</button>
...
// ─── Slash-Command-Menü ───────────────────────────────────────────────────────
function SlashMenu({ onPick, onClose, theme }) {
  useEffect(() => {
    const fn = e => { if (e.key === 'Escape') onClose(); };
    window.addEventListener('keydown', fn);
    return () => window.removeEventListener('keydown', fn);
  ...
  return (
    <div style={{
      border:'0.5px solid '+(theme?.border||'#E6E4DF'), borderRadius:10,
      background: theme?.surface || '#fff', boxShadow:'0 8px 24px rgba(0,0,0,.12)',
      overflow:'hidden', marginTop:4, marginBottom:6, maxWidth:280,
    }}>
      {SLASH_COMMANDS.map(cmd => (
        <div key={cmd.key} onClick={()=>onPick(cmd)}
          style={{display:'flex', alignItems:'center', gap:10, padding:'8px 12px', cursor:'pointer'}}
          onMouseEnter={e=>e.currentTarget.style.background = theme?.surface2 || '#FBFAF8'}
          onMouseLeave={e=>e.currentTarget.style.background = 'transparent'}>
          <span style={{fontSize:16, width:22, textAlign:'center'}}>{cmd.icon}</span>
          <div>
            <div style={{fontSize:13, fontWeight:500, color:theme?.text||'#1F1E1C'}}>{cmd.label}</div>
            <div style={{fontSize:11, color:theme?.textSoft||'#9B9994'}}>{cmd.desc}</div>
          </div>
        </div>
      ))}
    </div>
